  ...
  <table class="table display-no-order">
    ...
    <tbody>
      ...
      <tr class="{{ $warna }}">
        ...
        --}}
        <td>
          <span class="badge text-bg-light">{{ $at->invoice }}</span> {!! $status !!} <span class="badge {{$badge}}">{{
            $at->trx_type }}</span><br>
        ...
        <td class="text-center">
          ...
          <form action="{{ route('accounttrace.delete', $at->id) }}" method="post" class="d-inline">
            ...
            <button type="submit" onclick="this.disabled=true;this.value='Sending, please wait...';this.form.submit();"
              onclick="return confirm('Are you sure?')" class="btn btn-danger btn-sm"><i class="fa-solid fa-trash"></i>
            </button>
          </form>
        </td>
      </tr>
